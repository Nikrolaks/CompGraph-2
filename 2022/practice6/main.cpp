...
void main()
{
    if (mode != 2) {
        vec2 coord = texcoord;
        if (mode == 1)
            coord += vec2(sin(texcoord.y * 50.0 + time) * 0.01, 0.0);
        vec3 color = texture(render_result, coord).rgb;
        if (mode == 0)
...
int main() try
{
    ...
    auto render_loc = glGetUniformLocation(rectangle_program, "render_result");
    ...
    glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, frame, 0);
    glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, renbuf);

    if (glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        throw "pipets";

    auto calc_perspective = [&]() {
        float near = 0.1f;
        float far = 100.f;
        return glm::perspective(glm::pi<float>() / 2.f, (1.f * width) / height, near, far);
    };

    auto calc_projection = [&]() {
        float aspect_ratio = (float)(width) / height;
        return glm::ortho(-aspect_ratio, aspect_ratio, -1.f, 1.f, -1.5f, 1.5f);
    };

    auto draw = [&](int ind) {
        static const float cx[4] = {
            -0.5f, -0.5f, 0.5f, 0.5f
        };
        ...
        static const float colors[4][3] = {
            { 0.96875f, 0.828125f, 0.40625f },
            { 0.40625f, 0.96875f, 0.5546875f },
            { 0.40625f, 0.5546875f, 0.96875f },
            { 0.96875f, 0.40625f, 0.828125f }
        };
        
        static const float vecs[3][3] = {
            { 0, 0, 1 },
            { 1, 0, 0 },
            { 0, 1, 0 }
        };
        ...
        glClearColor(colors[ind][0], colors[ind][1], colors[ind][2], 0.f);
        ...
        if (ind < 3)
            view = glm::rotate(view, angles[ind], { vecs[ind][0], vecs[ind][1], vecs[ind][2] });
        else
            view = glm::rotate(view, view_angle, { 1.f, 0.f, 0.f });

        glm::vec3 camera_position = (glm::inverse(view) * glm::vec4(0.f, 0.f, 0.f, 1.f)).xyz();

        auto projection = ind < 3 ? calc_projection() : calc_perspective();
        ...
        glUniform2f(center_location, cx[ind], cy[ind]);
        glUniform2f(size_location, 0.5f, 0.5f);
        glUniform1i(mode_location, ind);
